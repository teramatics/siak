...
if(isset($_POST['tambah'])) {
  ...
  $mapel_level = serialize($_POST['mapel_level']);
  $mapel_kls = serialize($_POST['mapel_kls']);
  ...
  $pdo->setAttribute(PDO::ATTR_EMULATE_PREPARES, false);
  $query2 = $pdo->prepare('INSERT INTO profil (uid, nama, email, jk, tpl, tgl, alamat, kota, telp, masuk_kls, masuk_tgl, asal_sekolah, alamat_sekolah, tahun_ijazah, no_ijazah, unit, level, kelas, mapel, mapel_level, mapel_kls, posted) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)') or die("Gagal mengecek profil di database");
  $data_profil = array($nextid, $nama, $email, $jk, $tpl, $tgl, $alamat, $kota, $telp, $msk_kls, $msk_tgl, $asal_sekolah, $alamat_sekolah, $thn_ijazah, $no_ijazah, $unit, $level, $kelas, $mapel, $mapel_level, $mapel_kls, $posted);
  $query2->execute($data_profil);

  $direct = $base.'pengajar/'.$unit.'/'.$level.'/'.$kelas;
  redirect($direct);
}
...
<div class="control-group">
  ...
  <div class="controls">
    <?php 
      $query = $pdo->prepare("SELECT * FROM level WHERE lid <=2"); $query->execute(); 
      while ($row = $query->fetch(PDO::FETCH_ASSOC)) { 
    ?>
    <label class="checkbox inline">
      <input name="mapel_level[]" value="<?=$row['level']?>" type="checkbox">
      <?=$row['ket']?>
    </label>
    <?php } ?>
  </div>
...
<div class="control-group">
  ...
  <div class="controls">
    <?php 
      $query = $pdo->prepare("SELECT * FROM kelas WHERE kid <=2"); $query->execute(); 
      while ($row = $query->fetch(PDO::FETCH_ASSOC)) { 
    ?>
    <label class="checkbox inline">
      <input name="mapel_kls[]" value="<?=$row['kelas']?>" type="checkbox">
      <?=$row['ket']?>
    </label>
    <?php } ?>
  </div>
</div>
<?php } else if($unit == 'sd01' || $unit == 'sd02') { ?>
<!-- Multiple Checkboxes (inline) -->
<div class="control-group">
  <label class="control-label">Level</label>
  <div class="controls">
    <?php 
      $query = $pdo->prepare("SELECT * FROM level WHERE lid >=3"); $query->execute(); 
      while ($row = $query->fetch(PDO::FETCH_ASSOC)) { 
    ?>
    <label class="checkbox inline">
      <input name="mapel_level[]" value="<?=$row['level']?>" type="checkbox">
...
<?php } else if($unit == 'smp' || $unit == 'sma') { ?>
<!-- Multiple Checkboxes (inline) -->
<div class="control-group">
  <label class="control-label">Level</label>
  <div class="controls">
    <?php 
      $query = $pdo->prepare("SELECT * FROM level WHERE lid BETWEEN 3 AND 5"); $query->execute(); 
      while ($row = $query->fetch(PDO::FETCH_ASSOC)) { 
    ?>
    <label class="checkbox inline">
      <input name="mapel_level[]" value="<?=$row['level']?>" type="checkbox">
